...
public class AccountController {
    ...
    public ResponseEntity<AccountResponse> getStatement(
            @PathVariable
            @NotBlank(message = "<accountNumber> must not be null or blank.")
            @Pattern(regexp = "[1-9][0-9]{9}",
                    message = "<accountNumber> must be a 10-digit number starting with non-zero.")
            ...
            int size) {
        AccountResponse response = accountService.filter(accountNumber, from, to, page, size);
